...
problem: DIN scores (user, history, candidate_item) triples. Training uses a FULL
softmax loss over the entire item catalog (cross-entropy against every item, no
negative sampling) -- this matches the candidate space used at evaluation time
(ranking the full catalog for NDCG@10) and avoids a train/eval distribution
mismatch that sampled softmax with a small random-negative pool would introduce.
...
class DIN(nn.Module):
    def __init__(self, n_users, n_items, user_feat_vocabs, item_feat_vocabs, cfg: "Config"):
        ...
        # core id embeddings
        # NOTE: no raw per-uid embedding here on purpose. train/test uid sets are
        # disjoint (cold-start users at eval/serving time), so a plain uid->vector
        # lookup table would never be updated for test users and would just inject
        # untrained random noise into their representation. The user side is instead
        # represented purely by profile features (user_side_embs below, shared vocab
        # across train/test) plus the attention-pooled history interest computed in
        # encode_user() -- both of which generalize to uids never seen in training.
        ...
        # user representation is now side-feature-only (no id embedding term)
        user_full_dim = side_dim * len(user_feat_vocabs)
        if user_full_dim == 0:
            # defensive fallback in case a dataset ever has zero user side features;
            # keeps the user branch well-defined (a learned constant vector) rather
            # than crashing on an empty concat.
            self.user_bias = nn.Parameter(torch.zeros(side_dim))
            user_full_dim = side_dim
        else:
            self.user_bias = None
    ...
    def _user_full_emb(self, uid_idx):
        # NOTE: intentionally does not look up any id-based embedding for uid_idx --
        # see the comment in __init__. The user representation here is built purely
        # from profile features, so it's well-defined for uids never seen in training
        # (e.g. test-set users) as long as their feature *values* were seen.
        feats = self.user_feat_table[uid_idx]  # [B, n_u_cat]
        side_parts = [emb(feats[..., j]) for j, emb in enumerate(self.user_side_embs)]
        if not side_parts:
            # zero-user-side-feature edge case: broadcast the learned bias vector
            return self.user_bias.unsqueeze(0).expand(uid_idx.shape[0], -1)
...
class DINRanker(nn.Module):
    """Wraps DIN: trains with a FULL softmax over the entire item catalog using the
    user vector from DIN's attention pooling (with candidate = the positive target
    during training, which is the standard trick used at training time), and a
    separate static item-scoring embedding (i_static) used to score ALL items at
    both train and inference time, since true DIN attention is candidate-specific
    and we approximate full-catalog scoring via a learned static projection of
...
def full_softmax_loss(user_vec, target_idx, all_item_vecs):
    ...
    standard cross-entropy with the true target as the label. This is the exact
    softmax objective DIN/DeepFM-style ranking models are usually trained with
    when the catalog size is small/medium enough to fit a [B, n_items] logit
    matrix in memory; it removes the train/eval distribution mismatch that
    sampled softmax introduces (training only ever saw a handful of random
    negatives, while evaluation ranks against the full catalog).
    """
