...
independently (a small MLP "activation unit"), then sum-pools. BST instead:
  ...
  2. Adds learned positional embeddings (position = recency rank) plus a
     per-token frequency signal (log1p(item_seq_counts), projected and added
     to the token embedding) so the model still has access to "how many times
     has the user interacted with this item", which DIN fed into its
     activation unit explicitly.
...
class BSTDataset(Dataset):
    ...
    feature (log1p(count) projected and added to the token embedding) so
...
class BST(nn.Module):
    def __init__(self, n_users, n_items, user_feat_vocabs, item_feat_vocabs, cfg: "Config"):
        ...
        # frequency signal (0 vector on padding / target positions, which carry no count)
    ...
    def encode_user(self, uid_idx, hist, hist_cnt, hist_len, cand_item_idx):
        ...
        # frequency signal: log1p(count), 0 on padding positions; candidate gets no
        # count signal of its own (it isn't a historical interaction), so we feed 0.
