# -*- coding: utf-8 -*-
"""
ensemble_din_bst.py -- Score-level ensemble of a trained DIN ranker and a
trained BST ranker for the same next-item recommendation task.

Both din_model.py and bst_model.py train independently and each save a
checkpoint (din_best.pt / bst_best.pt) containing the FULL state_dict of
their respective Ranker wrapper (DINRanker / BSTRanker). This script:
  ...
  1. Dynamically imports both training scripts as modules (by file path, so
     ...
     --bst_script if they're not named din_model.py / bst_model.py).
  ...
  3. Reconstructs DINRanker / BSTRanker with each script's own default
     Config() (so architecture dims match what was actually trained: note
     DIN and BST may use a different max_seq_len, which is fine -- it only
     affects how each model's own Dataset pads/truncates history, not the
     shared item/user vocabularies), loads the checkpoint weights, sets eval
     mode.
  4. Reconstructs the EXACT SAME validation split each training script used
     ...
     so DIN and BST are evaluated on the identical set of (uid, target) rows
     -- this is what makes a row-by-row score fusion valid.
  5. Computes full-catalog scores from each model separately, evaluates each
     ...
     score matrices in two ways and reports NDCG@10 for each:
       a) weighted average of PER-ROW Z-SCORE NORMALIZED scores, swept over
          alpha in [0, 1] (alpha=1 -> pure DIN, alpha=0 -> pure BST) so you
          can see the full curve and pick the best blend.
...
---------------------------------------------
DIN's and BST's logits come from two independently-trained dot-product heads
with no shared scale -- one might routinely produce logits in [-5, 5] and the
other in [-50, 50]. Averaging raw scores would let whichever model happens to
have larger magnitude dominate the sum regardless of which model is actually
more confident/correct. Standardizing each model's score row to zero mean /
unit variance before combining puts both models on a comparable footing, the
same trick used in most production-style score-level ensembling.

Usage
-----
python ensemble_din_bst.py \
    --din_script din_model_2.py --din_ckpt ./out_din/din_best.pt \
    --bst_script bst_model_1.py --bst_ckpt ./out_bst/bst_best.pt \
...
def split_val_df(bundle, seed, val_frac):
    ...
    bst_model.py's train_model(): same shuffle seed + same val_frac on the
    same train_df produces the same set of (uid, target_iid, ...) rows in the
    same order, which is what lets us fuse DIN's and BST's per-row scores."""
...
def load_bst_model(bst_mod, bundle, ckpt_path, device):
    cfg = bst_mod.Config()  # architecture must match training-time defaults
    user_feat_vocabs = [len(bundle.u_encoders[c]) for c in bundle.u_cat_cols]
    item_feat_vocabs = [len(bundle.i_encoders[c]) for c in bundle.i_cat_cols]
    model = bst_mod.BSTRanker(bundle.n_users, bundle.n_items, user_feat_vocabs, item_feat_vocabs, cfg)
    model.set_feature_tables(
        torch.tensor(bundle.user_feat, dtype=torch.long),
        torch.tensor(bundle.item_feat, dtype=torch.long),
    )
...
# ========================================================================================
# 3. Per-model full-catalog scoring (mirrors each script's own eval-time
#    candidate convention: drive attention/target-token with the LAST history
#    item, never the unknown true target)
# ========================================================================================

@torch.no_grad()
def score_batches(model, loader, item_vecs_all, device):
    """Yields (uid_idx_np, target_np, scores_np[B, n_items]) per batch, with
    PAD masked out, for any model exposing forward(uid,hist,cnt,len,cand) and
    score_against_catalog(user_vec, item_vecs) -- both DINRanker and BSTRanker
    share this exact interface."""
    for batch in loader:
...
def collect_all_scores(model, df, uid_enc, iid_enc, max_seq_len, dataset_cls, collate_fn,
                        item_vecs_all, device, batch_size, has_target=True):
    ...
    for uid_np, target_np, scores_np in score_batches(model, loader, item_vecs_all, device):
...
def main():
    ...
    parser.add_argument("--bst_script", default="bst_model.py")
    parser.add_argument("--din_ckpt", default="./din_best.pt")
    parser.add_argument("--bst_ckpt", default="./bst_best.pt")
    ...
    bst_mod = import_module_from_path("bst_model_user", args.bst_script)
    ...
    bst_model, bst_cfg = load_bst_model(bst_mod, bundle, args.bst_ckpt, device)
    print(f"[INFO] DIN max_seq_len={din_cfg.max_seq_len}  BST max_seq_len={bst_cfg.max_seq_len}")
    ...
    with torch.no_grad():
        ...
        bst_item_vecs = bst_model.item_static_vec(all_item_ids)
    ...
    uids_bst, targets_bst, scores_bst = collect_all_scores(
        bst_model, val_df, bundle.uid_enc, bundle.iid_enc, bst_cfg.max_seq_len,
        bst_mod.BSTDataset, bst_mod.collate_fn, bst_item_vecs, device, args.batch_size,
    )
    ...
    assert np.array_equal(uids_din, uids_bst), \
        "uid order mismatch between DIN and BST scoring passes -- check that both " \
        "models were trained on the same data_dir/seed/val_frac."
    assert np.array_equal(targets_din, targets_bst), "target mismatch between DIN and BST scoring passes."
    targets = targets_din

    ndcg_din = ndcg_at_k_matrix(scores_din, targets, k=args.topk)
    ndcg_bst = ndcg_at_k_matrix(scores_bst, targets, k=args.topk)
    print(f"[RESULT] DIN-only  valid_ndcg@{args.topk} = {ndcg_din:.4f}")
    print(f"[RESULT] BST-only  valid_ndcg@{args.topk} = {ndcg_bst:.4f}")

    z_din = zscore_rows(scores_din)
    z_bst = zscore_rows(scores_bst)

    best_alpha, best_ndcg = None, -1.0
    if args.fusion == "sweep":
        print(f"[INFO] sweeping weighted-fusion alpha (1.0=pure DIN, 0.0=pure BST)...")
        for alpha in np.round(np.arange(0.0, 1.01, 0.1), 2):
            fused = weighted_fuse(z_din, z_bst, alpha)
        ...
        rrf_scores = rrf_fuse(scores_din, scores_bst, k=args.rrf_k)
        ...
        fused = weighted_fuse(z_din, z_bst, args.alpha)
    ...
    print(f"\n[SUMMARY] DIN-only={ndcg_din:.4f}  BST-only={ndcg_bst:.4f}  "
          f"Ensemble({args.fusion})={best_ndcg:.4f}  "
          f"(improvement over best single model: {best_ndcg - max(ndcg_din, ndcg_bst):+.4f})")
    ...
    uids_bst_t, _, scores_bst_t = collect_all_scores(
        bst_model, test_df, bundle.uid_enc, bundle.iid_enc, bst_cfg.max_seq_len,
        bst_mod.BSTDataset, bst_mod.collate_fn, bst_item_vecs, device, args.batch_size,
        has_target=False,
    )
    assert np.array_equal(uids_din_t, uids_bst_t), "uid order mismatch on test.csv between DIN and BST."

    if args.fusion == "rrf":
        fused_t = rrf_fuse(scores_din_t, scores_bst_t, k=args.rrf_k)
    else:
        alpha = best_alpha if best_alpha is not None else args.alpha
        fused_t = weighted_fuse(zscore_rows(scores_din_t), zscore_rows(scores_bst_t), alpha)
