...
┌──────────────────────────────────────────────────────────┐
│  SeqEncoder（SASRec 风格因果 Transformer）                │
│  seq_ids → item_id_emb(E) + 4域特征均值(E) → proj(D)     │
│          + 位置编码(D)                                    │
│  → n_layers 因果自注意力块                                │
│  → 取最后有效位置输出 → seq_repr (B, D)                   │
│  → 序列内最后 k 个 item 的平均 → recent_repr (B, D)       │
│    (捕捉近期兴趣，与 seq_repr 互补)                        │
└──────────────────────────────────────────────────────────┘
           ↓ seq_repr + recent_repr
┌──────────────────────────────────────────────────────────┐
│  特征拼接层                                               │
│  concat[seq_repr(D); recent_repr(D);                     │
│         用户8域emb(8E); last_item_feat_emb(4E)]           │
│  → DCNv2 输入向量 x_0  (B, 2D+8E+4E)                    │
└──────────────────────────────────────────────────────────┘
           ↓
┌──────────────────────────────────────────────────────────┐
│  DCNv2（并行 Cross + Deep）                              │
│                                                          │
│  Cross Network v2（n_cross 层，低秩矩阵）                 │
│    x_{l+1} = x_0 ⊙ (U_l(V_l^T x_l) + b_l) + x_l        │
│    → 显式高阶特征交叉（阶数 = n_cross+1）                  │
│    输出 cross_out (B, 2D+8E+4E)                          │
│                                                          │
│  Deep Network（并行 MLP）                                 │
│    x → Linear-GELU-Dropout × n → deep_out (B, deep[-1]) │
│                                                          │
│  concat[cross_out; deep_out] → Linear → LayerNorm        │
│  → user_repr (B, D)                                      │
└──────────────────────────────────────────────────────────┘
           ↓ user_repr
┌──────────────────────────────────────────────────────────┐
│  Item Tower（独立 embedding，不与 SeqEncoder 共享）        │
│  item_id(E) + 4域特征(4E) + FM二阶(E) → MLP → (B, D)    │
└──────────────────────────────────────────────────────────┘
           ↓
  打分：cosine(user_repr, item_repr) × exp(log_temp)
  训练：全量 Softmax CE（label smoothing）
       + Masked Item Prediction 辅助损失 × aux_weight

相比 Transformer+DCNv2+CL 版本的简化
--------------------------------------
- 去掉对比学习（ProjectionHead + InfoNCE + 序列增强）
- 训练每 step 只做一次 forward，速度约快 3×
- 增加 recent_repr（近期 k 个 item 均值）作为 DCNv2 额外输入，
  弥补去掉 CL 后对近期兴趣建模的损失
- 增加 last_item_feat_emb（最后一个交互 item 的特征 embedding），
  让 DCNv2 能直接感知最近交互的 item 侧特征
"""
...
class Config:
    ...
    output_path   = "submission_tdcn.csv"

    # 序列
    max_seq_len   = 50
    emb_dim       = 32        # 每个特征域 embedding 维度 E
    repr_dim      = 128       # 主干隐层维度 D
    recent_k      = 5         # recent_repr 使用最后 k 个有效 item
    ...
    deep_dims     = [512, 256, 128]   # Deep Network 各层宽度
    ...
    # 辅助任务
    aux_weight    = 0.1       # Masked Item Prediction 损失权重
    mask_prob     = 0.15      # 序列 mask 比例
...
class SeqEncoder(nn.Module):
    """
    行为序列 → (seq_repr, recent_repr, seq_hidden, last_item_ids)

    seq_repr   : 最后位置 Transformer 输出，捕捉全局历史兴趣
    recent_repr: 最后 recent_k 个有效 item 的 hidden 均值，捕捉近期兴趣
    seq_hidden : 全序列 hidden states，供辅助任务使用
    last_item_ids: 最后一个有效 item 的 id（用于抽取 item 侧特征）
    """
    ...
    def forward(self, seq_ids, ift):
        ...
        # token embedding = item_id(E) + 4域特征均值(E) → proj(D)
        id_e   = self.item_id_emb(seq_ids)                          # (B,L,E)
        feats  = ift[seq_ids]                                        # (B,L,4)
        f_mean = torch.stack([self.feat_embs[j](feats[:,:,j])
                               for j in range(4)], dim=-2).mean(-2) # (B,L,E)
        ...
        # seq_repr：取最后位置（左padding → index L-1 是最新 item）
        seq_repr = torch.nan_to_num(x[:, -1, :], nan=0.0)           # (B,D)

        # recent_repr：最后 recent_k 个有效位置的 hidden 均值
        # 左 padding 时最后 recent_k 个位置即 x[:, L-recent_k:, :]
        recent_x    = x[:, -self.recent_k:, :]                      # (B,k,D)
        recent_mask = pad_mask[:, -self.recent_k:]                   # (B,k)
        valid       = (~recent_mask).float().unsqueeze(-1)           # (B,k,1)
        recent_repr = (recent_x * valid).sum(1) / valid.sum(1).clamp(min=1)  # (B,D)
        recent_repr = torch.nan_to_num(recent_repr, nan=0.0)

        # last_item_ids：最后一个非 PAD 的 item id（用于抽取 item 特征）
        # 左 padding → 从右向左找第一个非 0
        valid_mask   = (seq_ids != 0).long()                         # (B,L)
        last_pos     = valid_mask.flip(1).argmax(1)                  # (B,) 翻转后的位置
        last_abs_pos = L - 1 - last_pos                              # (B,) 原始位置
        last_item_ids = seq_ids[torch.arange(B, device=device), last_abs_pos]  # (B,)

        return seq_repr, recent_repr, x, last_item_ids


# ── 3b. DCNv2 ──────────────────────────────────────────────
class CrossNetV2(nn.Module):
    """
    DCN v2 Cross Network，低秩矩阵权重版本。
    每层：x_{l+1} = x_0 ⊙ (U_l (V_l^T x_l) + b_l) + x_l
    U_l, V_l ∈ R^{d×r}，r = max(1, d//4)
    n_cross 层 → 显式 n_cross+1 阶特征交叉
    """
    def __init__(self, d, n_cross):
        super().__init__()
        r = max(1, d // 4)
        self.Us = nn.ParameterList([nn.Parameter(torch.empty(d, r)) for _ in range(n_cross)])
        self.Vs = nn.ParameterList([nn.Parameter(torch.empty(d, r)) for _ in range(n_cross)])
        self.bs = nn.ParameterList([nn.Parameter(torch.zeros(d))    for _ in range(n_cross)])
        for i in range(n_cross):
            nn.init.xavier_normal_(self.Us[i])
            nn.init.xavier_normal_(self.Vs[i])

    def forward(self, x):
        x0 = x
        for U, V, b in zip(self.Us, self.Vs, self.bs):
            # x V → (B,r)；再乘 U^T → (B,d)
            x = x0 * (x @ V @ U.T + b) + x
        return x
...
class DCNv2(nn.Module):
    """
    并行 Cross Network + Deep Network → concat → Linear → LayerNorm → user_repr

    in_dim   : 输入拼接特征维度
    n_cross  : Cross Network 层数
    deep_dims: Deep Network 各层宽度（最后一层作为 deep_out 维度）
    out_dim  : 最终输出维度（= repr_dim）
    """
    def __init__(self, in_dim, n_cross, deep_dims, out_dim, dropout):
        super().__init__()
        self.cross    = CrossNetV2(in_dim, n_cross)
        self.deep     = build_mlp(in_dim, deep_dims[:-1], deep_dims[-1], dropout)
        # cross_out(in_dim) + deep_out(deep_dims[-1]) → out_dim
        self.out_proj = nn.Linear(in_dim + deep_dims[-1], out_dim)
    ...
    def forward(self, x):
        ...
        out = self.norm(F.gelu(self.out_proj(self.drop(combined))))  # (B, out_dim)
...
class ItemTower(nn.Module):
    ...
    独立 embedding，不与 SeqEncoder 共享（避免梯度冲突）
    """
    def __init__(self, cfg, n_items, item_feat_dims, item_feat_padidxs):
        super().__init__()
        ...
        self.feat_embs   = nn.ModuleList([
            nn.Embedding(dim, E, padding_idx=pidx)
            for dim, pidx in zip(item_feat_dims, item_feat_padidxs)])
        # id(E) + feat_concat(4E) + FM二阶(E) → MLP → D
        self.mlp = build_mlp(E + E*4 + E, cfg.item_mlp_dims, D, cfg.dropout)
...
# 4. 主模型
# ══════════════════════════════════════════════════════════
class TransformerDCNv2(nn.Module):
    """
    Transformer + DCNv2 端到端推荐模型

    DCNv2 的输入设计（充分利用所有特征）：
      concat[
        seq_repr     (D)    ← Transformer 全局序列表示
        recent_repr  (D)    ← 近期 k 个 item 的均值表示
        user_feat_emb(8E)   ← 用户8个特征域 embedding 拼接
        last_item_emb(4E)   ← 最后交互 item 的4个特征域 embedding
      ]
      = 2D + 8E + 4E

    Cross Network 对这个拼接向量做显式高阶交叉，
    让 seq_repr × user_feat、recent_repr × item_feat 等组合
    在每一层都被显式建模。

    损失：
      L = L_main（全量 Softmax CE）+ aux_weight × L_aux（Masked Item Pred）
    """
    def __init__(self, cfg, n_items,
        ...
        # ── last item 的 4 个特征域 embedding（与 SeqEncoder 共享 feat_embs）──
        # 单独建一套，保持独立
        self.last_feat_embs = nn.ModuleList([
            nn.Embedding(dim, E, padding_idx=pidx)
            for dim, pidx in zip(item_feat_dims, item_feat_padidxs)])

        # ── DCNv2 ──
        dcn_in = D + D + E*8 + E*4     # seq_repr + recent_repr + user_feat + last_item_feat
        self.dcn = DCNv2(
            in_dim    = dcn_in,
            n_cross   = cfg.n_cross,
    ...
    def _aux_loss(self, seq_hidden, mask_targets):
        ...
        return F.cross_entropy(torch.clamp(self.aux_head(hidden), -50, 50), tgt)
    ...
    def encode_user(self, seq_ids, ift, user_feats):
        ...
        # 序列编码
        seq_repr, recent_repr, seq_hidden, last_ids = self.seq_enc(seq_ids, ift)

        # 用户特征 embedding 拼接  (B, 8E)
        u_cat = torch.cat([e(user_feats[:,j]) for j,e in enumerate(self.u_embs)], -1)

        # 最后一个 item 的 4 个特征域 embedding 拼接  (B, 4E)
        last_feats = ift[last_ids]                              # (B, 4)
        li_cat = torch.cat(
            [self.last_feat_embs[j](last_feats[:,j]) for j in range(4)], -1)  # (B,4E)

        # DCNv2 输入拼接
        dcn_in    = torch.cat([seq_repr, recent_repr, u_cat, li_cat], -1)  # (B, 2D+8E+4E)
        user_repr = self.dcn(dcn_in)                            # (B, D)
    ...
    def forward(self, seq_ids, masked_seq_ids, ift, user_feats,
        ...
        if mixup_alpha > 0:
            main_loss = lam*main_loss + (1-lam)*self._ce(logits[:,1:], target_ids_b-1)
