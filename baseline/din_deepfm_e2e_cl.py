...
class Config:
    ...
    repr_dim    = 256         # user/item tower 输出维度
    ...
    cl_weight = 0.1  # 对比学习 Loss 的权重，建议从 0.1 或 0.01 开始尝试
    temp = 0.05      # 温度系数，用于缩放 logits
...
class DINDeepFME2E(nn.Module):
    ...
    def forward(self, seq_ids, ift, user_feats, target_ids=None):
        ...
        # 2. 归一化 (可选，但推荐用于对比学习，相当于计算余弦相似度)
        # 如果不归一化，就是普通的点积缩放
        user_repr_norm = F.normalize(user_repr, p=2, dim=-1)
        item_repr_norm = F.normalize(item_repr, p=2, dim=-1)

        # 3. 计算相似度矩阵 (B, N+1)
        # 加入温度系数 temp
        logits = torch.matmul(user_repr_norm, item_repr_norm.T) / cfg.temp
        ...
        if target_ids is not None:
            ...
            # --- 原始 Loss (CrossEntropy) ---
            # 注意：target_ids 是 (B,)，logits 是 (B, N+1)
            # 你的原始代码里 target_ids 是原始 ID，这里假设 target_ids 已经是 index
            # 如果 target_ids 是原始 iid，需要减 1 (因为 padding 是 0)
            loss_ce = F.cross_entropy(logits[:, 1:], target_ids - 1)

            # --- 新增：对比学习 Loss (InfoNCE) ---
            # 思路：构造一个 BxB 的矩阵，对角线是正样本
            # 取出 target item 的 embedding (B, D)
            target_item_repr = self.item_tower(target_ids, ift[target_ids]) # (B, D)
            target_item_repr_norm = F.normalize(target_item_repr, p=2, dim=-1)

            # 计算 User 与 Target Item 的相似度 (B,)
            pos_sim = torch.sum(user_repr_norm * target_item_repr_norm, dim=-1) # (B,)
            pos_sim = pos_sim / cfg.temp

            # 计算 User 与 Batch 内所有 Item 的相似度 (B, B)
            # 这里利用矩阵乘法计算 Batch 内对比
            batch_item_repr = item_repr[target_ids] # (B, D)，取出当前 Batch 对应的 Item 向量
            batch_item_repr_norm = F.normalize(batch_item_repr, p=2, dim=-1)

            sim_matrix = torch.matmul(user_repr_norm, batch_item_repr_norm.T) / cfg.temp # (B, B)
