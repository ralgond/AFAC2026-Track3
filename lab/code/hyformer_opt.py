...
class RecDataset(Dataset):
    ...
    def _mask(self, seq_t: torch.Tensor):
        """
        向量化 mask：用 torch.rand 一次性生成所有位置的概率，
        避免 Python 逐元素循环，对 num_workers > 0 的 worker 进程天然线程安全。
        seq_t : (L,) LongTensor，已 pad
        return: masked (L,), tgt (L,)  均为 LongTensor
        """
        non_pad  = seq_t != 0                                  # (L,) bool
        do_mask  = torch.rand(seq_t.shape) < self.mask_prob    # (L,) bool
        mask_pos = non_pad & do_mask                           # (L,) bool

        tgt    = torch.where(mask_pos, seq_t,
                             torch.zeros_like(seq_t))          # (L,)
        masked = torch.where(mask_pos,
                             torch.full_like(seq_t, self.mask_id),
                             seq_t)                            # (L,)
        return masked, tgt

    def __getitem__(self, idx):
        s     = self.samples[idx]
        seq_t = torch.tensor(pad_seq(s["seq"], self.max_len), dtype=torch.long)
        uf    = torch.tensor(s["user_feat"], dtype=torch.long)
        if self.mode == "train":
            masked, mtgt = self._mask(seq_t)
            return (seq_t,
                    masked,
                    uf,
                    torch.tensor(s["target"], dtype=torch.long),
                    mtgt)
        return seq_t, uf, s["uid"]
...
def main():
    ...
    # num_workers > 0：数据预取与 GPU 计算并行，消除"GPU 等 CPU"的 stall
    # pin_memory=True：worker 直接写入锁页内存，加速 CPU->GPU 的 DMA 拷贝
    # persistent_workers=True：epoch 间不销毁/重建 worker 进程，减少启动开销
    ...
    tr_ld = DataLoader(tr_ds, shuffle=True,  **_loader_kwargs)
    va_ld = DataLoader(va_ds, shuffle=False, **_loader_kwargs)
    te_ld = DataLoader(te_ds, shuffle=False, **_loader_kwargs)
