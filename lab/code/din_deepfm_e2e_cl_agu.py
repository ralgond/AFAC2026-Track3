...
def augment_sequence(seq, drop_ratio=0.2, mask_ratio=0.1, mask_idx=0):
    """
    对输入序列进行数据增强
    seq: (B, L) 的 tensor
    drop_ratio: 随机丢弃 item 的比例
    mask_ratio: 随机用 mask_idx 替换 item 的比例
    mask_idx: 用于替换的 mask token id，通常为 0 (padding)
    """
    seq_len = seq.size(1)
    aug_seq = seq.clone()

    # 1. 随机丢弃 (Random Dropout)
    if drop_ratio > 0:
        # 生成一个与 seq 形状相同的随机矩阵
        drop_probs = torch.rand_like(seq.float())
        # 生成 drop mask，注意 padding 位置 (idx=0) 不参与丢弃
        drop_mask = (drop_probs > drop_ratio) | (seq == mask_idx)
        aug_seq = aug_seq * drop_mask

    # 2. 随机遮盖 (Random Masking)
    if mask_ratio > 0:
        mask_probs = torch.rand_like(seq.float())
        # 生成 mask，同样 padding 位置不参与
        mask_condition = (mask_probs < mask_ratio) & (seq != mask_idx)
        # 使用 torch.where 进行替换
        aug_seq = torch.where(mask_condition, torch.tensor(mask_idx, device=seq.device), aug_seq)
...
class UserTower(nn.Module):
    ...
    def forward(self, seq_ids, user_feats, seq_ids_aug=None):
        """
        seq_ids: (B, L) 原始序列
        user_feats: (B, 8) 用户特征
        seq_ids_aug: (B, L) 增强后的序列，训练对比学习时传入
        """
        B, L = seq_ids.shape
        device = seq_ids.device
        ...
        # --- 处理增强序列 (如果提供了的话) ---
        if seq_ids_aug is not None:
            # 对增强序列做同样的 embedding 和 pooling 操作
            seq_e_aug = self.drop(self.item_id_emb(seq_ids_aug) + self.pos_emb(pos))
            seq_d_aug = self.drop(F.gelu(self.seq_proj(seq_e_aug)))
            pad_mask_aug = (seq_ids_aug == 0)
            valid_aug = (~pad_mask_aug).float().unsqueeze(-1)
            hist_vec_aug = (seq_d_aug * valid_aug).sum(1) / valid_aug.sum(1).clamp(min=1)
            # 注意：DIN 部分我们仍然使用原始序列的 user_feat_vec 作为 query
            # 这样可以保证兴趣提取的稳定性，只对历史行为做扰动
            din_vec_aug = self.din(user_feat_vec, seq_d_aug, pad_mask_aug)
        else:
            hist_vec_aug, din_vec_aug = None, None

        # --- 生成最终表征 ---
        # 原始序列的表征
        concat = torch.cat([user_feat_vec, hist_vec, din_vec, user_fm], dim=-1)
        user_repr = self.out_mlp(concat)

        if seq_ids_aug is not None:
            # 增强序列的表征
            concat_aug = torch.cat([user_feat_vec, hist_vec_aug, din_vec_aug, user_fm], dim=-1)
            user_repr_aug = self.out_mlp(concat_aug)
            return user_repr, user_repr_aug # 训练时，返回两个表征
        else:
            return user_repr, None # 推理时，返回 (表征, None)
...
class DINDeepFME2E(nn.Module):
    ...
    def forward(self, seq_ids, ift, user_feats, target_ids=None):
        ...
        # 1. 数据增强
        if self.training:
            seq_ids_view1 = augment_sequence(seq_ids, drop_ratio=0.2, mask_ratio=0.1)
            seq_ids_view2 = augment_sequence(seq_ids, drop_ratio=0.2, mask_ratio=0.1)
        else:
            # 推理时，两个视图都设为原始序列，第二个视图实际上不会被使用
            seq_ids_view1, seq_ids_view2 = seq_ids, None

        # 2. 获取用户表征
        # 现在无论训练还是推理，都能正确解包
        user_repr, user_repr_aug = self.user_tower(seq_ids_view1, user_feats, seq_ids_view2)

        # 3. 获取物品表征
        all_ids = torch.arange(self.n_items + 1, device=device)
        item_repr = self.item_tower(all_ids, ift)
        
        # 4. 计算推荐任务的 Loss
        logits = user_repr @ item_repr.T
        
        if target_ids is not None:
            loss_rec = F.cross_entropy(logits[:, 1:], target_ids - 1)
            
            # 5. 计算对比学习的 Loss
            # 只有在训练时，user_repr_aug 才不为 None
            if user_repr_aug is not None:
                user_repr_norm = F.normalize(user_repr, p=2, dim=-1)
                user_repr_aug_norm = F.normalize(user_repr_aug, p=2, dim=-1)
                
                sim_matrix = torch.matmul(user_repr_norm, user_repr_aug_norm.T) / cfg.temp
                labels = torch.arange(user_repr.size(0), device=device)
            ...
            # 6. 总 Loss
            loss = loss_rec + cfg.cl_weight * loss_cl
            return loss
...
def main():
    ...
    val_indices = set(random.sample(range(len(train_samples)), n_val))
    val_samps = [s for i, s in enumerate(train_samples) if     i in val_indices]
    tr_samps  = [s for i, s in enumerate(train_samples) if not i in val_indices]
    print(f"  Val={len(val_samps)} | Train={len(tr_samps)}\n")
