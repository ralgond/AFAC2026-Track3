...
历史修改轨迹由多个节点组成，整体是一棵树，节点的有parent_id属性，id属性，is_leaf属性。parent_id为当前节点的父节点的id，is_leaf为true时表示该节点是一个叶子，也即是一条trajectory的tail。给定一个叶子节点id，通过追溯它的parent_id直到parent_id为-1，也即是根节点，可得到一个trajectory。你应该构建一棵树，方便后面的分析和生成操作。
{history_trajectory}
...
- qb_n_heads和n_heads必须是4的倍数，repr_dim必须是qb_n_heads和n_heads的倍数。
...
def main():
    ...
    shutil.copy("./code/hyformer.py", f"{final_path}/model.py")
